...
class Feature(SubModule):
    def __init__(self):
        ...
        state_dict = torch.load('mobilenetv2_100_pretrained.pth')
        model.load_state_dict(state_dict, strict=False)
        layers = [1, 2, 3, 5, 6]
        chans = [16, 24, 32, 96, 160]
        # self.conv_stem = model.conv_stem
        self.conv_stem = nn.Conv2d(1, model.conv_stem.out_channels, kernel_size=model.conv_stem.kernel_size,
                                   stride=model.conv_stem.stride, padding=model.conv_stem.padding, bias=False)
        self.conv_stem.weight.data = model.conv_stem.weight.data.mean(dim=1, keepdim=True)
        self.bn1 = model.bn1
